...
TEST (CeresSolver, HelloWorld1) {
    ...
    // T.ODO 1: почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    // Потому что это производная функции блока невязки, а не решаемой задачи. То есть, если в блоке невязки лежит
    // f(x), то мы минимизируем 0.5 * f(x)^2, а её производная -- это как раз 0.5 * (f'(x) * f(x) + f(x) * f'(x)) = f(x) * f'(x) = 0
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // T.ODO 3: раскомментируйте^, почему он находит не то что ожидалось?
        ...
        // Там была ошибка в поиске расстояния до параболоида, стр. 137
    }

    // T.ODO 4: если любопытно и хватит времени - можете попросить ceres-solver посчитать якобианы в некоторых точках подобно тому как это делалось в конце теста HelloWorld1
    // и сверить что найденные аналитически на бумажке результаты совпадают (через ASSERT_NEAR)
    // Ошибка и без этого нашлась
}
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // T.ODO 5 посчитайте единственную невязку - расстояние от нашей точки-замера до текущего состояния прямой (для извлечения корня, помня про T=Jet, нужно использовать ceres::sqrt):
        ...
        T q = ceres::sqrt(ceres::pow(line[0], 2) + ceres::pow(line[1], 2));
        residual[0] = (line[0] * samplePoint[0] + line[1] * samplePoint[1] + line[2]) / q;
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        // T.ODO 6 удалите этот return сразу после выполнения T.ODO 5
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // T.ODO 7 расскоментируйте сверку найденной прямой и эталонной
        ...
        // T.ODO 7 поправьте тест так или иначе (хотя бы пост-процессингом)

        // Как минимум надо как-то отнормировать прямые, потому что, очевидно, одну и ту же прямую можно задать разными числовыми параметрами
        // Первый вариант -- поделить на всё на С, второй вариант -- на sqrt(a^2 + b^2)
        // Вообще тут, конечно подходит больше праметрическое задание прямой.
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 * (1.f - outliers_fraction)); // T.ODO 8 раскоментируйте, почему эта проверка падает? как поправить?
    // Мы сами сделали данные шумными, очевидно нельзя требовать от идеальной прямой полной идеальности
    // Сделаем эту проверку динамической, надо чтобы она обратно зависела от количества шума
    ASSERT_LT(mse, 1.1 * sigma * sigma); // T.ODO 9 раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    // Падает на тестах с сильно шумными данными, а всё потому что расстояние знаковое и надо смотреть на его модуль
    ...
    if (outliers_fraction == 0 || use_huber) {
        // T.ODO 10 раскоментируйте обе проверки, почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
        ASSERT_GT(inliers_fraction, 0.99 * (1.f - outliers_fraction));
