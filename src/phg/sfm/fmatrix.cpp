...
//      Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        auto sv = svdf.singularValues();

        F = svdf.matrixU() * Eigen::DiagonalMatrix<double, 3>(sv[0], sv[1], 0) * svdf.matrixV().transpose();
    ...
    {
        cv::Matx33d tr = cv::Matx33d::zeros();

        for (int i = 0; i < 2; i++) {
            double mean = 0.f;
            for (int j = 0; j < m.size(); j++) mean += m[j][i];
            mean /= m.size();

            double var = 0.f;
            for (int j = 0; j < m.size(); j++) var += (m[j][i] - mean) * (m[j][i] - mean);
            var /= (double)m.size();

            double c = std::sqrt(2 / var);
