                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    {
        if (matches[i].size() == 1) filtered_matches.push_back(matches[i][0]);
        else if (matches[i][0].distance < K_RATIO * matches[i][1].distance)
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; i++) {
        std::set<int> near_indices;

        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) >= radius2_query) break;
            near_indices.insert(indices_query.at<int>(i, j));
        }

        int matches_cnt = 0;
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_train.at<float>(i, j) >= radius2_train) break;
            if (near_indices.find(indices_train.at<int>(i, j)) != near_indices.end()) matches_cnt++;
        }

        if (matches_cnt >= consistent_matches) filtered_matches.push_back(matches[i]);
    }
}
