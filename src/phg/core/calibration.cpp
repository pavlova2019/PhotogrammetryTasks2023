...
cv::Vec3d phg::Calibration::project(const cv::Vec3d &point) const
{
    ...
    // T.ODO 11: добавьте учет радиальных искажений (k1_, k2_) (после деления на Z, но до умножения на f)

    double q = x * x + y * y;
    double add = k1_ * q + k2_ * q * q;
    x += add;
    y += add;
...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    // T.ODO 12: добавьте учет радиальных искажений, когда реализуете - подумайте: почему строго говоря это - не симметричная формула формуле из project? (но лишь приближение)
    // Тут уже искажёння точка, а там всё считали до искажения. Так что это не симметрично.
